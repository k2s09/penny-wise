...
export class AuthService {
  ...
  onSignUp(values: any):Promise<boolean> {
    ...
    let body = {
      ...
      category:['Transportation','Groceries','Entertainment','Unassigned','Bills','Health','Shopping'],
    };
    this.http.post(this.apiUrl + 'USER/SIGN_UP', body).subscribe(
      (res: any) => {
        if (res) {
          this._snackBar.open(
            'Pennywise Account Created SuccessFully',
            '',
            { duration: 4000 }
          );
  ...
  saveAllData(body:any){
    this.http.post(this.apiUrl+'SAVE_DATA',body).subscribe((res:any)=>{
      this._snackBar.open('Pennywise Account Created SuccessFully','',{duration:2000});
    })
  }
